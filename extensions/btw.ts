...
class BtwOverlay extends Container implements Focusable {
	// Scroll constants
	private static readonly SCROLL_AMOUNT_LINES = 3;
	private static readonly SCROLL_AMOUNT_PAGE_RATIO = 0.5;
	...
	private handleScroll(data: string): boolean {
		const innerWidth = this.calculateInnerWidth(this.currentWidth);
		const lines = this.getTranscript(innerWidth, this.theme);
		const totalLines = lines.length;
		const visibleArea = this.getVisibleAreaHeight();
		const maxOffset = Math.max(0, totalLines - visibleArea);
		...
		if (this.keybindings.matches(data, "tui.select.up")) {
			if (this.scrollOffset < maxOffset) {
				this.scrollOffset = Math.min(this.scrollOffset + BtwOverlay.SCROLL_AMOUNT_LINES, maxOffset);
				needsRender = true;
			}
		} else if (this.keybindings.matches(data, "tui.select.down")) {
			if (this.scrollOffset > 0) {
				this.scrollOffset = Math.max(this.scrollOffset - BtwOverlay.SCROLL_AMOUNT_LINES, 0);
				needsRender = true;
			}
		} else if (this.keybindings.matches(data, "tui.select.pageUp")) {
			if (this.scrollOffset < maxOffset) {
				const pageAmount = Math.max(1, Math.floor(visibleArea * BtwOverlay.SCROLL_AMOUNT_PAGE_RATIO));
				this.scrollOffset = Math.min(this.scrollOffset + pageAmount, maxOffset);
				needsRender = true;
			}
		} else if (this.keybindings.matches(data, "tui.select.pageDown")) {
			if (this.scrollOffset > 0) {
				const pageAmount = Math.max(1, Math.floor(visibleArea * BtwOverlay.SCROLL_AMOUNT_PAGE_RATIO));
				this.scrollOffset = Math.max(this.scrollOffset - pageAmount, 0);
				needsRender = true;
			}
		} else if (this.keybindings.matches(data, "tui.editor.cursorLineStart")) {
			if (this.scrollOffset !== 0) {
				this.scrollOffset = 0;
				needsRender = true;
			}
		} else if (this.keybindings.matches(data, "tui.editor.cursorLineEnd")) {
			if (this.scrollOffset !== maxOffset) {
		...
		if (needsRender) {
			...
			// Also update input's focus state for proper cursor rendering
			this.input.focused = this._focused;
		}
		return needsRender;
	}

	private getVisibleAreaHeight(): number {
		const dialogHeight = this.calculateDialogHeight();
		return Math.max(OVERLAY.transcriptMinLines, dialogHeight - OVERLAY.chromeHeight);
	}
	...
	// Layout helpers
	private calculateInnerWidth(width: number): number {
		return Math.max(40, width - 2);
	}

	private calculateDialogHeight(): number {
		const terminalRows = process.stdout.rows ?? 30;
		return Math.max(OVERLAY.dialogHeightMin, Math.min(OVERLAY.dialogHeightMax, Math.floor(terminalRows * OVERLAY.heightPct)));
	}

	private getVisibleLines(allLines: string[], visibleArea: number): { visibleLines: string[]; hasMoreAbove: boolean; hasMoreBelow: boolean } {
		const totalLines = allLines.length;
		if (totalLines <= visibleArea) {
			this.scrollOffset = 0;
			return { visibleLines: allLines, hasMoreAbove: false, hasMoreBelow: false };
		}

		// Clamp scroll offset (lines hidden at bottom)
		const maxOffset = totalLines - visibleArea;
		if (this.scrollOffset > maxOffset) this.scrollOffset = maxOffset;
		if (this.scrollOffset < 0) this.scrollOffset = 0;

		const start = totalLines - visibleArea - this.scrollOffset;
		const end = start + visibleArea;
		const visibleLines = allLines.slice(Math.max(0, start), end);
	...
	private renderHeader(innerWidth: number): string {
		return this.frameLine(this.theme.fg("accent", this.theme.bold(" BTW side chat ")), innerWidth);
	}
	...
	private renderTranscript(lines: string[], innerWidth: number, hasMoreAbove: boolean, hasMoreBelow: boolean): string[] {
		const rendered: string[] = [];

		if (hasMoreAbove) {
			rendered.push(this.frameLine(this.theme.fg("dim", " ↑ more above"), innerWidth));
		}

		rendered.push(...lines.map((line) => this.frameLine(line, innerWidth)));

		if (hasMoreBelow) {
			rendered.push(this.frameLine(this.theme.fg("dim", " ↓ more below"), innerWidth));
		}

		return rendered;
	}

	private renderStatus(innerWidth: number, status: string): string {
		return this.frameLine(this.theme.fg("warning", status), innerWidth);
	}

	private renderInputLine(innerWidth: number): string {
		const promptWidth = visibleWidth(OVERLAY.inputPrompt);
		const inputWidth = innerWidth - promptWidth;
		const inputParts = this.input.render(inputWidth);
		const inputContent = inputParts[0] ?? "";
		return `${OVERLAY.inputPrompt}${inputContent}`;
	}

	private renderInput(inputLine: string, innerWidth: number): string {
		return this.frameLine(inputLine, innerWidth);
	}

	private renderHelp(innerWidth: number): string {
		return this.frameLine(this.theme.fg("dim", "Enter: submit · Esc: close · Arrows/PgUp/PgDn: scroll"), innerWidth);
	}

	private renderFooter(innerWidth: number): string {
		return this.borderLine(innerWidth, "bottom");
	}

	override render(width: number): string[] {
		this.currentWidth = width;
		const innerWidth = this.calculateInnerWidth(width);
		const dialogHeight = this.calculateDialogHeight();
		const transcriptHeight = Math.max(OVERLAY.transcriptMinLines, dialogHeight - OVERLAY.chromeHeight);

		// Get full transcript and apply scrolling
		const allLines = this.getTranscript(innerWidth, this.theme);
		const { visibleLines, hasMoreAbove, hasMoreBelow } = this.getVisibleLines(allLines, transcriptHeight);
		...
		lines.push(...this.renderTranscript(visibleLines, innerWidth, hasMoreAbove, hasMoreBelow));
		lines.push(this.renderSeparator(innerWidth));
		lines.push(this.renderStatus(innerWidth, status));
		lines.push(this.renderInput(inputLine, innerWidth));
		lines.push(this.renderHelp(innerWidth));
...
export default function (pi: ExtensionAPI) {
	...
	function getTranscriptLinesInner(width: number, theme: ExtensionContext["ui"]["theme"]): string[] {
		...
		const lines: string[] = [];
		// Render all past thread items (no truncation; scrolling handles overflow)
		for (const item of thread) {
			// User message - render full multi-line
			const questionLines = item.question.trimEnd().split("\n");
			for (let i = 0; i < questionLines.length; i++) {
				const prefix = i === 0 ? theme.fg("accent", theme.bold("You: ")) : "      ";
				const maxWidth = width - visibleWidth(prefix);
				const line = truncateToWidth(questionLines[i], maxWidth > 0 ? maxWidth : 1, "…");
		...
		// Render pending (current) interaction if any
		if (pendingQuestion) {
			const questionLines = pendingQuestion.trimEnd().split("\n");
			for (let i = 0; i < questionLines.length; i++) {
				const prefix = i === 0 ? theme.fg("accent", theme.bold("You: ")) : "      ";
				const maxWidth = width - visibleWidth(prefix);
				const line = truncateToWidth(questionLines[i], maxWidth > 0 ? maxWidth : 1, "…");
