...
1. **Tech stack**: "What's the tech stack? (language, framework, runtime)"
   → Provides context for the Commands section

1. **Build command**: "What's the exact build command?"
   Options: `pnpm build`, `npm run build`, `make`, `cargo build`, `go build ./...`, `npm run dev` (if no build step), `other`, `none`

1. **Test command**: "What's the exact test command?"
   Options: `pnpm test`, `npm test`, `cargo test`, `go test ./...`, `pytest`, `other`, `none`

1. **Lint/format command**: "Any linting or formatting commands?"
...
1. **MEMORY.md**: "Do you have a MEMORY.md in this project?"
...
1. **PLAN.md/todo.md**: "Do you have a PLAN.md or todo.md for task tracking?"
...
1. **Boundaries**: "Are there any boundaries the agent should follow? (things to never do, files to never touch, security-sensitive areas, areas where it should ask permission)"

1. **PR conventions**: "Any PR or commit conventions? (title format, required checks, review process)"

1. **Code style**: "Any code style preferences? (naming conventions, patterns to follow or avoid)"
